...
/*
	Redactor Plugin: filemanager.js
*/
...
RedactorPlugins.filemanager = function () {
    return {
        init: function () {
            ...
            this.button.setAwesome('filemanager', 'fa-picture-o');
        },
        show: function () {
            this.modal.addTemplate('filemanager', this.filemanager.getTemplate());

            this.modal.load('filemanager', 'File Manager', 900);

            this.modal.createCancelButton();

            var button = this.modal.createActionButton('Insert');
            button.attr("id", "btnSelectImage");
            button.hide();
            button.on('click', this.filemanager.insert);

            jQuery("#redactor-modal header,#redactor-modal-close").remove();
            jQuery(".redactor-modal-close-btn").hide();

            this.selection.save();
            this.modal.show();

            jQuery('#mymodal-textarea').focus();
        },
        getTemplate: function () {
            return String()
            + '<section id="redactor-modal-filemanager">'
            + '<div id="modal_prog_loading" style="display: block;"><div class="spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div></div>'
            + '<iframe id="ifrmRedactorFileManager" src="file_manager.asp?fileaction=wysiwyg" style="width: 100%; height: 600px; overflow: hidden; padding: 0;" frameborder="0"></iframe>'
            + '</section>';
        },
        insert: function () {
        	var strSelectedFilePath = "";
			if (document.getElementById("ifrmRedactorFileManager").contentDocument.getElementById("hdnSelectedImage").value.substring(0,4) == "http")
				strSelectedFilePath = document.getElementById("ifrmRedactorFileManager").contentDocument.getElementById("hdnSelectedImage").value;
			else
	            strSelectedFilePath = "/" + document.getElementById("ifrmRedactorFileManager").contentDocument.getElementById("hdnSelectedImage").value;

            var arrAux = strSelectedFilePath.split("/");
            var strSelectedFileName = arrAux[arrAux.length-1];
            this.modal.close();
            this.selection.restore();
            this.insert.html("<img src=\"" + strSelectedFilePath + "\" alt=\"" + strSelectedFileName + "\" />");
            this.code.sync();
        }
    };
};
...
(function ($) {
    RedactorPlugins.fontfamily = function () {
        return {
            init: function () {
            	var fonts = ['Arial', 'Helvetica', 'Georgia', 'Times New Roman', 'Monospace', 'Verdana'];
                var that = this;
                var dropdown = {};
...
(function ($) {
	...
	$.Redactor.opts = {
		...
		plugins: ['table', 'fontfamily', 'fontsize', 'fontcolor', 'filemanager', 'video', 'fullscreen'], // edit: array or false
		...
		linebreaks: false,
		replaceDivs: false, // edit: true
		paragraphize: false, // edit: true
		cleanStyleOnEnter: false,
		enterKey: true,

		cleanOnPaste: false, // edit: true
		cleanSpaces: true,
		...
		convertLinks: true,
		convertUrlLinks: false,   // edit: true
		convertImageLinks: false, // edit: true
		...
		removeWithoutAttr: false, // edit: ['span']
		...
		clean: function () {
			return {
				onSet: function (html) {
					...
					// edit: convert style tag
					html = html.replace(/<style(.*?[^>]?)>([\w\W]*?)<\/style>/gi, '<pre class="redactor-style-tag" style="display: none;" $1>$2</pre>');

					// edit: convert input tag
					html = html.replace(/<input(.*?[^>]?)>([\w\W]*?)/gi, '<pre class="redactor-input-tag" style="display: none;" $1>$2</pre>');

					// edit: convert textarea tag
					html = html.replace(/<textarea(.*?[^>]?)>([\w\W]*?)<\/textarea>/gi, '<pre class="redactor-textarea-tag" style="display: none;" $1>$2</pre>');

					// edit: convert select tag
					html = html.replace(/<select(.*?[^>]?)>([\w\W]*?)<\/select>/gi, '<pre class="redactor-select-tag" style="display: none;" $1>$2</pre>');

					// edit: convert base tag
					html = html.replace(/<base(.*?[^>]?)>([\w\W]*?)/gi, '<pre class="redactor-base-tag" style="display: none;" $1>$2</pre>');
				...
				onSync: function (html) {
					...
					// edit: reconvert style tag
					html = html.replace(/<pre class="redactor-style-tag" style="display: none;"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<style$1>$2</style>');

					// edit: reconvert input tag
					html = html.replace(/<pre class="redactor-input-tag" style="display: none;"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<input$1>$2');

					// edit: reconvert textarea tag
					html = html.replace(/<pre class="redactor-textarea-tag" style="display: none;"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<textarea$1>$2</textarea>');

					// edit: reconvert select tag
					html = html.replace(/<pre class="redactor-select-tag" style="display: none;"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<select$1>$2</select>');

					// edit: reconvert base tag
					html = html.replace(/<pre class="redactor-base-tag" style="display: none;"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<base$1>$2');
